...
export default function ObasVoiceTypeFieldPage() {
  ...
  function startMeter() {
    ...
    const tick = () => {
      ...
      setMetrics((m) => ({ rms: m.rms * 0.62 + rms * 0.38, peak: Math.max(peak, m.peak * 0.82), attack: m.attack * 0.5 + attack * 0.5, pause, rate: m.rate * 0.94 }));
  ...
  useEffect(() => {
    ...
    const lines: string[] = [];
    if (words.length <= 2) lines.push(display);
    else {
  ...
  return (
    <main style={{ minHeight: '100vh', position: 'relative', overflow: 'hidden', background: '#050505', color: '#f4f4f4', fontFamily: 'ui-monospace, SFMono-Regular, Menlo, monospace' }}>
      <section style={{ minHeight: '88vh', display: 'grid', placeItems: 'center', padding: '28px 18px 90px' }}>
        <canvas ref={canvasRef} aria-label={text || 'Live voice transcription'} style={{ width: 'min(96vw, 1440px)', height: 'auto', aspectRatio: '16 / 9', display: 'block', imageRendering: 'pixelated' }} />
      </section>
      <div style={{ position: 'absolute', left: 16, right: 16, bottom: 14, display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap', fontSize: 10, letterSpacing: '.08em', textTransform: 'uppercase', color: '#8c8c8c' }}>
        <div><div>{status}</div><div style={{ marginTop: 4 }}>LEVEL {energy.toFixed(2)} // ATTACK {attack.toFixed(2)} // RATE {metrics.rate.toFixed(2)} // PAUSE {metrics.pause.toFixed(2)}</div></div>
        <button type="button" onClick={active || shouldListenRef.current ? stopVoiceField : startVoiceField} disabled={!supported} style={{ minHeight: 44, padding: '0 14px', border: '1px solid #555', background: '#050505', color: '#d8d8d8', font: 'inherit', cursor: supported ? 'pointer' : 'not-allowed', opacity: supported ? 1 : 0.45 }}>
